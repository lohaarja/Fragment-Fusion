...
def main():
    ...
    with st.sidebar:
        ...
        if uploaded_files:
            # Check if new files are uploaded (different from previous upload)
            current_files_count = len(uploaded_files)
            if current_files_count != st.session_state.uploaded_files_count:
                # Reset processing state when new files are uploaded
                reset_processing_state()
                st.session_state.uploaded_files_count = current_files_count
            
            # Save uploaded files to a temporary directory
            temp_dir = "temp_uploads"
            
            # Clean up existing temp directory first
            cleanup_temp_directory(temp_dir)
            os.makedirs(temp_dir, exist_ok=True)
            
            # Save new files with error handling
            saved_files = []
